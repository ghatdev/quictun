...
pub fn run(local_addr: SocketAddr, setup: EndpointSetup, dpdk_config: DpdkConfig) -> Result<()> {
    ...
    if dpdk_config.mode == "tap" {
        ...
        // Virtio-user: single vdev with 1 queue pair regardless of n_cores.
        // Multi-queue virtio-user causes kernel TAP to distribute RX across queues,
        // but DPDK only reads queue 0 → packet loss. Workers use inner_tx ring instead.
