...
impl BbrState {
    ...
    /// Record a sent data packet (tracked for CC: inflight, size, timestamp).
    ...
    /// Register an ACK-only packet in sent_bitmap with size=0.
    ///
    /// ACK-only packets bypass CC (no inflight tracking) but must still be
    /// registered so that on_ack can resolve them. Without this, ACK-only PNs
    /// create gaps in sent_bitmap; advance_base skips over these gaps and makes
    /// data PNs permanently unresolvable, causing an inflight deadlock.
    pub fn register_ack_only(&self, tracker: &SentTracker, pn: u64) {
        let idx = (pn as usize) & RING_MASK;
        tracker.sent_sizes[idx].store(0, Ordering::Relaxed);
        tracker.sent_times[idx].store(0, Ordering::Relaxed);
        tracker.sent_bitmap.set(pn);
    }
    ...
    pub fn on_ack(&self, tracker: &SentTracker, ack: &AckFrame, now_ns: u64) {
        ...
        let mut skip_already_acked: u64 = 0;
        let mut skip_not_sent: u64 = 0;

        // Process each acknowledged range
        for range in &ack.ranges {
            for pn in range.start..range.end {
                // Skip PNs not in sent_bitmap (ACK-only packets, or TX task
                // hasn't called on_sent yet). Do NOT set acked_bitmap for
                // unsent PNs — the TX task may still be in-flight between
                // encrypt_datagram() and on_sent(), and prematurely setting
                // the acked bit would cause the PN to be skipped forever.
    ...
    fn update_phase(&self) {
        ...
        // BDP floor: never go below INITIAL_CWND. This prevents bdp=0 from
        // trapping DRAIN forever (exit requires inflight <= bdp).
        let bdp = if min_rtt < u64::MAX && max_bw > 0 {
            ((max_bw * min_rtt) / 1_000_000_000).max(INITIAL_CWND)
        } else {
            INITIAL_CWND
        };
        ...
        match phase {
            p if p == BbrPhase::Startup as u64 => {
                ...
                // Require meaningful data delivered to avoid transitioning on
                // control traffic (tiny packets with large intervals give wrong BW).
                let delivered = self.delivered.load(Ordering::Relaxed);
                if max_bw > 0 && delivered >= INITIAL_CWND && inflight >= bdp {
                    tracing::debug!(
                        inflight, bdp, max_bw, min_rtt, delivered,
                        new_cwnd = ((bdp * BBR_DRAIN_GAIN) / 256).max(MIN_CWND),
                        "BBR STARTUP -> DRAIN"
                    );
            ...
            p if p == BbrPhase::Drain as u64 => {
                // DRAIN: reduce cwnd to drain the queue built during STARTUP.
                // Sends are blocked (inflight > cwnd) until ACKs drain inflight.
                // The RX task sends ACK-only packets to maintain feedback.
                let new_cwnd = ((bdp * BBR_DRAIN_GAIN) / 256).max(MIN_CWND);
                self.cwnd.store(new_cwnd, Ordering::Relaxed);
                self.pacing_rate.store((max_bw * BBR_DRAIN_GAIN) / 256, Ordering::Relaxed);

                if inflight <= bdp {
                    tracing::debug!(inflight, bdp, new_cwnd, "BBR DRAIN -> PROBE_BW");
    ...
    /// Get the current BBR phase name for logging.
    pub fn phase_name(&self) -> &'static str {
        match self.phase.load(Ordering::Relaxed) {
            p if p == BbrPhase::Startup as u64 => "startup",
            p if p == BbrPhase::Drain as u64 => "drain",
            _ => "probe_bw",
        }
    }
...
/// (TX task) and on_ack() (RX task) share the same baseline. A thread-local
/// epoch breaks RTT measurement when TX and RX run on different threads.
///
